...
export default function Projects() {
  return (
    <section
      id="projects"
      className="relative min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-gray-100 to-white px-6 py-20 overflow-hidden"
    >
      {/* 🔮 Background blobs */}
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-20">
          {completedProjects.map((project, idx) => (
            <motion.div
              ...
              className="bg-white/40 backdrop-blur-md border border-white/20 shadow-lg rounded-xl p-6 space-y-4 hover:shadow-xl transition-shadow"
            >
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {upcomingProjects.map((project, idx) => (
            <motion.div
              ...
              whileHover={{ scale: 1.03 }}
              transition={{ duration: 0.3 }}
              className="bg-white/30 backdrop-blur-md border border-white/20 shadow-lg rounded-xl p-6 space-y-4 hover:shadow-xl transition-shadow"
            >
              <h3 className="text-2xl font-semibold text-gray-800">
                {project.title}
              </h3>
              ...
              {project.github && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block mt-4 text-gray-700 hover:text-black text-xl"
                >
                  <FaGithub />
                </a>
              )}
